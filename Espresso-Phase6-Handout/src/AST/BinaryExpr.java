...
public class BinaryExpr extends Expression {
	...
	return left().isConstant() && right().isConstant() && (op().kind != BinOp.INSTANCEOF);
    }
	...
	if (left().constantValue() instanceof String && right().constantValue() instanceof String && op().kind == BinOp.PLUS) {
	    String l = (String)left().constantValue();
	    String r = (String)right().constantValue();
	    return "\"" + l.substring(1, l.length() - 1) + r.substring(1, r.length() - 1) + "\"";
	} else if (left().constantValue() instanceof String && !(right().constantValue() instanceof String) && op().kind == BinOp.PLUS) {
	    String l = (String)left().constantValue();
	    BigDecimal rval = (BigDecimal) right().constantValue();
	    return "\"" + l.substring(1, l.length() - 1) + rval + "\"";
	} else if (!(left().constantValue() instanceof String) && right().constantValue() instanceof String && op().kind == BinOp.PLUS) {
	    BigDecimal lval = (BigDecimal) left().constantValue();
	    String r = (String)right().constantValue();
	    return "\"" + lval + r.substring(1, r.length() - 1) + "\"";	    
	}
	BigDecimal lval = (BigDecimal) left().constantValue();
	BigDecimal rval = (BigDecimal) right().constantValue();
	...
	case BinOp.DIV:   
	    ...
	    return new BigDecimal(lval.doubleValue()/rval.doubleValue());
	case BinOp.MOD: 
	case BinOp.LSHIFT:
	case BinOp.RSHIFT:
	case BinOp.RRSHIFT:
