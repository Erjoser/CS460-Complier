...
public class Generator {
	...
	if (x == 0.00F)
	    classFile.addInstruction(new Instruction(RuntimeConstants.opc_fconst_0));
	else if (x == 1.00F)
	    classFile.addInstruction(new Instruction(RuntimeConstants.opc_fconst_1));
	else if (x == 2.00F)
	    classFile.addInstruction(new Instruction(RuntimeConstants.opc_fconst_1));
	else
	    classFile.addInstruction(new LdcFloatInstruction(RuntimeConstants.opc_ldc, x, s));
    }
	...
	if (x == 0.00)
	    classFile.addInstruction(new Instruction(RuntimeConstants.opc_dconst_0));
	else if (x == 1.00)
	    classFile.addInstruction(new Instruction(RuntimeConstants.opc_dconst_1));
	else
	    classFile.addInstruction(new LdcDoubleInstruction(RuntimeConstants.opc_ldc2_w, x, s));
    }
	...
	// if the address is less than 4 we can generate Xload_Y (Y=address).
	String instString;
	if (storeArrayRef) {
	    String prefix = "";
	    ...
	    instString = prefix + "astore";
	} else {
	    if (address < 4)
		instString = type.getTypePrefix() + "store_" + address;
	    else
		instString = type.getTypePrefix() + "store";
	}
	...
	if (loadArrayRef) {
	    ...
	    instString = prefix + "aload";
	} else {
	    if (address < 4)
		instString = type.getTypePrefix() + "load_" + address;
	    else
		instString = type.getTypePrefix() + "load";
	}	
	return Generator.getOpCodeFromString(instString);				    
    }
     ...
     * @param doubleDup (dup2, dup2_x1, dup2_x2)
     */
