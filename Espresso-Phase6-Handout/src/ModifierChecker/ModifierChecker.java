...
public class ModifierChecker extends Visitor {
	...
	public Object visitFieldDecl(FieldDecl fd) {
		...
		if (fd.modifiers.isFinal() && fd.var().init() == null)
		    Error.error(fd, "Field '" + fd.name() + "' in " + (currentClass.isInterface() ? "interface" : "class") + " '" +
	...
	// Cannot assing to .length
	if (leftHandSide && fr.target().type.isArrayType() && fr.fieldName().getname().equals("length"))
	    Error.error(fr,"cannot assign a value to final variable length.");
	...
	if ((fr.target() instanceof This)
	    ...
	    //&& !fd.modifiers.isStatic()) -- test - removed on 4/9/24 just for fun
	...
	if (fr.target().isClassName() && !(fd.modifiers.isStatic()))
	...
	if (leftHandSide && fd.modifiers.isFinal())
        ...
        boolean oldLeftHandSide = leftHandSide;
	// If the target is fieldRef then _it_ is not being assigned to; i.e., it is not really
	// on the left-hand side, so set leftHandSide to false.
	...
	public Object visitInvocation(Invocation in) {
	    ...
	    //<--
	    // 12/06/13 .length() or .charAt for Strings
