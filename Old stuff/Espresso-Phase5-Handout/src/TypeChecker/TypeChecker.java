...
public class TypeChecker extends Visitor {
	...
	case BinOp.NOTEQ:{
		...
		be.type = new PrimitiveType(PrimitiveType.BooleanKind);
	    else if (lType.isClassType() && rType.isClassType()) {
		if (Type.isSuper((ClassType)lType, (ClassType)rType) ||
		    Type.isSuper((ClassType)rType, (ClassType)lType))
		    be.type = new PrimitiveType(PrimitiveType.BooleanKind);
		else
	...
	case BinOp.MOD: {
		...
		if (lType.isVoidType() || rType.isVoidType())
		    Error.error("Operand for + cannot be void type.");
	...
	for (int i=0; i<actualParamCount; i++) {
	    ...
	    // No references to this or super in an explicit constructor invocation's arguments.
	    // No direct 'this' or a field reference with a taget of 'this' or 'super'.
	    if (ap instanceof This) 
		Error.error(ci, "Cannot reference `this' before supertype constructor has been called");
	    if (ap instanceof FieldRef && ((FieldRef)ap).target() instanceof This)
		Error.error(ci, "Cannot reference `this' before supertype constructor has been called");
	    if (ap instanceof FieldRef && ((FieldRef)ap).target() instanceof Super)
		Error.error(ci, "Cannot reference `super' before supertype constructor has been called");
	...
	if (constructor == null) 
	    Error.error(ci,"No constructor " + targetClass.name() + "(" + Type.parseSignature(s) + " ) found.\nCandidates are:" +
	...
	// Check if there are any recursive (direct or indirect) constructor calls (skip Object).
	if (!cd.name().equals("Object")) {
	    for (int i=0;i<cd.body().nchildren;i++) {
		// for each constructor
		if (cd.body().children[i] instanceof ConstructorDecl) {
		    ConstructorDecl cdd = (ConstructorDecl)cd.body().children[i];
		    ConstructorDecl original = cdd;
		    // iterate through the chain of 'this' constructors. A 'super' constructor breaks the chain.
		    while (cdd.cinvocation().thisConstructorCall()) {
			ConstructorDecl cddd = cdd.cinvocation().constructor;
			if (original == cddd)
			    Error.error(cdd, "Recursive constructor invocation");
			cdd = cdd.cinvocation().constructor;
		    }
		}
	    }
	}
	...
	*/
	...
	if (in.target() == null) {
		...
		if (!t.isIntegralType() || t.isLongType()) 
	...
	// Method not found.
	if (method == null) 
	    Error.error(in, "No method " + methodName + " (" + Type.parseSignature(s) + " ) found. \nCandidates are:" +
	...
	*/
	...
	if (constructor == null) 
	    Error.error(ne, "No constructor " + cd.name() + "(" + Type.parseSignature(s) + " ) found.\nCandidates are:" +
	...
	for (i=0;i<ss.switchBlocks().nchildren; i++) {
		...
		if (sl.expr().isConstant()) {
		    if (eType.isShortType() && Literal.isShortValue(((BigDecimal)sl.expr().constantValue()).longValue()))
			continue;
		    if (eType.isByteType() && Literal.isByteValue(((BigDecimal)sl.expr().constantValue()).longValue()))
			continue;
		    if (eType.isCharType() && Literal.isCharValue(((BigDecimal)sl.expr().constantValue()).longValue()))
			continue;
		}
				
		if (!Type.assignmentCompatible(eType, lType))
		    Error.error(sl, "Incompatible types: possible lossy conversion from " + lType.typeName() + " to " + eType.typeName());
	...
	for (i=0;i<ss.switchBlocks().nchildren; i++) {
		...
		if (eType.isStringType() && !sl.isDefault())
		    ...
		    strval = ((BigDecimal)sl.expr().constantValue()).toString();
	...
	// if there is only a superclass return it.
	if (interfaces.nchildren == 0) {
	    return superClass;
	}
