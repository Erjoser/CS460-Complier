...
public abstract class Type extends AST {
	...
	if (signature().equals(other.signature()))	    
	    ...
	    return true;
	else if ((this instanceof ArrayType) && (other instanceof NullType))
	    return true;
	else if ((this instanceof NullType) && (other instanceof ArrayType))
	    return true;
	else
	    return false;
    }
	...
	if (var.identical(val)) // Same type
	    ...
	    return true;
	else if ((var instanceof ArrayType) && (val instanceof ArrayType)) {
	    ArrayType at1 = (ArrayType)var;
	    ArrayType at2 = (ArrayType)val;
	    if (at1.baseType().isClassType() && at2.baseType().isClassType() &&
		isSuper((ClassType)at1.baseType(), (ClassType)at2.baseType()))
		return true;	
	}
	return false;
    }
